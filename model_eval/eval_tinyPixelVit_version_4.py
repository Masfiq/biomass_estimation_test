# eval for tinyPixelViT_version_3 — 5x5 patches, version_5 CSV, Huber-trained
    ...
    def __getitem__(self, idx):
        ...
        if self.sentinel1_lookup is not None:
            try:
                ...
                x_img_np[self.FIXED_BANDS.index("S1_VV")] = s1_patch[0]
                x_img_np[self.FIXED_BANDS.index("S1_VH")] = s1_patch[1]
                x_img = torch.from_numpy(x_img_np)
            except Exception as e:
                print(f"[S1 ERROR __getitem__] idx={idx}: {e}", flush=True)

        x_aux_np = np.concatenate(
            [m_sc, koppen_onehot, gridmet_features], axis=0
        ).astype(np.float32)
...
def main():
    ...
    ap.add_argument("--ckpt", default="/s/chopin/e/proj/hyperspec/masfiq/models/tinyPixelViT_version_4_fusion_geohash_month_koppen_SAR_version_5_California_North_10_2021.pth")
    ap.add_argument("--val-frac", type=float, default=0.2)
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--batch-size", type=int, default=256)
    ap.add_argument("--geohash-precision", type=int, default=7)
    ap.add_argument("--out", default="tinyPixelViT_version_4_eval_version_5.json")
