# eval for tinyUnet_version_8 — 5x5 patches, version_6 CSV, Huber-trained
# path variables are in the bash .sh file
# Copied from eval_tinyUnet_version_7.py. Every edit is tagged CHANGE A or CHANGE B so
# it can be diffed against that file.
#
# ---- CHANGE A: DEM terrain features (4 new aux inputs) ----------------------------
# tinyUnet_version_8_train.py feeds 4 extra AUXILIARY features per shot, read from the
# Copernicus GLO-30 mosaic: [elev_km, slope_norm, aspect_sin, aspect_cos]. They are aux
# features, NOT image channels, so in_channels is unchanged at 14 while aux_dim grows
# from 2+30+6=38 to 42. The DEMLookup class below is copied verbatim from the train
# script -- if the two ever diverge the checkpoint will load but score nonsense.
#
# ---- CHANGE B: sqrt target instead of log1p --------------------------------------
# target_col "agbd_log" -> "agbd_sqrt", so the back-transform to Mg/ha is x**2 rather
# than the inverse-log used in version_7. Applied at all three places version_7 did it.
# Negative predictions are clamped to 0 first: squaring a negative would silently turn
# an underprediction into a large POSITIVE biomass, which the old transform never did.
#
# version_8's training run saved a "_best" checkpoint (epoch 056, Val Huber 4.0115)
# separately from the final-epoch one (epoch 100, Val Huber 4.0852), so this eval
# defaults to the _best checkpoint rather than the plain OUTPUT_PATH one.
# NOTE: Huber values are NOT comparable to version_7's -- the target space changed and
# HuberLoss delta was rescaled 1.0 -> 5.0 to match the wider sqrt range.
...
class GEDIHlsPatchDatasetFusion(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x_aux_np = np.concatenate(
            [m_sc, koppen_onehot, gridmet_features, dem_features], axis=0
        ).astype(np.float32)
