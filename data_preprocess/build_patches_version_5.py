# Building 5x5 patches for HLS (8 bands) + NLCD (1 band). No SAR bands in this version.
# v5 changes vs v4:
# (1) Patch window is now 5x5 (150m x 150m) instead of 3x3 (90m x 90m)
# (2) We still predict the CENTER pixel biomass — agbd_center / agbd_log unchanged
# (3) Resume support added: skips shots whose patch file already exists
# (4) Incremental CSV flush every 500 patches to survive preemption
...
PATCHES_ROOT = "/s/chopin/e/proj/hyperspec/masfiq/dataset/patch_tifs_california_north_10_2021_AprilToAugust_version_5"

OUT_CSV_PATH = "/s/chopin/e/proj/hyperspec/masfiq/csv_files/gedi_california_north_10_2021_AprilToAugust_version_5.csv"
...
def extract_5x5_patch_for_all_bands(
    ...
    Final output (no SAR): 8 HLS bands + 1 NLCD = 9 bands, each 5x5.
    """
    if sensor == "S30":
    ...
    # 3. Sentinel-1 VV and VH (skipped when S1_ROOT=None)
...
def _process_one_shot(task):
    ...
    try:
        ...
        # Still predicting center pixel biomass, same as v3/v4
...
def build_gedi_hls_patches_multiprocess(
    gedi_folder,
    hls_root,
    patches_root="patch_tifs_version_5",
    out_csv_path="gedi_hls_metadata_version_5.csv",
    min_quality=1,
    nlcd_path=None,
    s1_root=None,
    max_s1_days_diff=12,
    ...
    # 4) Index Sentinel-1 scenes (skipped when S1_ROOT=None)
    s1_index = None
