# tinyPixelViT version 6
# version 5 + best-validation checkpointing. version_5's run showed validation Huber
# bottoming out at epoch 96 (0.2563) then drifting to 0.2593 by epoch 100, but the
# script only ever called torch.save() once, at the very end, so the checkpoint it
# kept was the worse, overfit one. This version saves a separate "_best" checkpoint
# every time validation Huber improves, so the best epoch is never lost to whatever
# the last epoch happens to be. The final-epoch model is still saved to OUTPUT_PATH
# as before, for continuity with earlier scripts.
...
OUTPUT_PATH = Path("/s/chopin/e/proj/hyperspec/masfiq/models/tinyPixelViT_version_6_fusion_geohash_month_koppen_SAR_version_6_California_North_10_2021.pth")
