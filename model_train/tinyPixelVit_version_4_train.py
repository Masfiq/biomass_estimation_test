# tinyPixelViT version 4
# version 3 + SAR (Sentinel-1 VV/VH)
...
OUTPUT_PATH = Path("/s/chopin/e/proj/hyperspec/masfiq/models/tinyPixelViT_version_4_fusion_geohash_month_koppen_SAR_version_5_California_North_10_2021.pth")
...
class Sentinel1Lookup:
    ...
    def __getstate__(self):
        """Never pickle open rasterio handles across process boundaries."""
        d = self.__dict__.copy()
        d["_src_cache"] = {}
        d["_cache_pid"] = None
        return d
    ...
    def _normalize_s1_patch(self, arr: np.ndarray) -> np.ndarray:
        ...
        return np.clip(arr_norm, 0.0, 1.0).astype(np.float32)

    def _get_src(self, tif_path):
        """
        Return (dataset, transform, pyproj_transformer) for a scene, cached per
        worker process.

        The previous version reopened the GeoTIFF, re-derived its spatial ref and
        built a fresh pyproj Transformer on EVERY sample -- ~500k times per epoch
        across only 26 distinct files. That churn leaked ~0.35 MB/sample of
        anonymous memory and OOM-killed the job mid-epoch. There are 26 scenes, so
        this cache is bounded at 26 entries.
        """
        pid = os.getpid()
        if getattr(self, "_cache_pid", None) != pid:
            # New worker process: never inherit handles across a fork.
            self._src_cache = {}
            self._cache_pid = pid

        key = str(tif_path)
        ent = self._src_cache.get(key)
        if ent is None:
            ...
            if CRS.from_user_input(raster_crs).to_epsg() == 4326:
                ...
                tf = Transformer.from_crs("EPSG:4326", raster_crs, always_xy=True)
            ent = (src, raster_transform, tf)
            self._src_cache[key] = ent
        return ent

    def _read_patch_from_tif(self, tif_path: Path, lon: float, lat: float):
        src, raster_transform, tf = self._get_src(tif_path)

        x, y = (lon, lat) if tf is None else tf.transform(lon, lat)
        ...
        # Clamp the window to the raster and pad manually rather than using
        # boundless=True, which constructs a throwaway in-memory VRT per call.
        r0, c0 = row - 2, col - 2
        r1, c1 = r0 + 5, c0 + 5
        rr0, cc0 = max(0, r0), max(0, c0)
        rr1, cc1 = min(src.height, r1), min(src.width, c1)

        patch = np.zeros((5, 5), dtype=np.float32)
        if rr1 > rr0 and cc1 > cc0:
            sub = src.read(1, window=Window(cc0, rr0, cc1 - cc0, rr1 - rr0))
            patch[rr0 - r0: rr1 - r0, cc0 - c0: cc1 - c0] = sub

        return patch

    def sample_patch(self, lon: float, lat: float, date_value: np.datetime64) -> np.ndarray:
        """Returns s1_patch of shape (2, 5, 5): channel 0 = VV, channel 1 = VH."""
        if len(self.scenes) == 0:
            print("[S1 ERROR] No Sentinel-1 VV/VH scenes found.", flush=True)
            return np.zeros((2, 5, 5), dtype=np.float32)
        ...
        for scene in scenes_sorted[:10]:
            try:
                ...
                if self._should_print():
                    print(f"[S1 WARNING] Could not use scene date={scene['date']}. Error: {e}", flush=True)
                continue
        print(
