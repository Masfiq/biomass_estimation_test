...
def create_dataloaders(csv_path, batch_size=32, val_frac=0.2, geohash_precision=7, use_koppen=True, seed=42):
    ...
    ds = GEDIHlsPatchDatasetFusion(
        ...
        koppen_legend_path=koppen_legend
    )

    # IMPORTANT for ablation fairness: keep the SAME train/val split every run
    ...
    print("Sample x_img:", x_img.shape, "in_channels=", x_img.shape[0])
    print("Sample x_aux:", x_aux.shape, "aux_dim=", x_aux.shape[0])  # should be 2 when use_koppen=False
...
def train_model(
    ...
    val_frac=val_frac,
    geohash_precision=geohash_precision,
    use_koppen=False,     # <-- ablation: NO KOPPEN
    seed=42
    )
...
if __name__ == "__main__":
    ...
    out_path = Path("/s/chopin/e/proj/hyperspec/masfiq/models/resnet18_fusion_geohash_month_NO_KOPPEN_field_boundary.pth")
