# resnet18 version 14
# version 13 + best-validation checkpointing. version_13's run showed real overfitting:
# validation Huber bottomed out at epoch 17 (0.3243) and drifted back up to 0.3464 by
# epoch 100, but the script only ever called torch.save() once, at the very end, so the
# checkpoint it kept was the worse, overfit one. This version saves a separate "_best"
# checkpoint every time validation Huber improves, so the best epoch is never lost to
# whatever the last epoch happens to be. The final-epoch model is still saved to
# OUTPUT_PATH as before, for continuity with earlier scripts.
...
OUTPUT_PATH = Path("/s/chopin/e/proj/hyperspec/masfiq/models/resnet18_version_14_fusion_geohash_month_koppen_withAttentionLayer_SAR_version_6_California_North_10_2021.pth")
