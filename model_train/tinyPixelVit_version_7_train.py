# tinyPixelViT version 7
# version 6 + two changes: (A) Copernicus DEM terrain aux features, (B) sqrt target
# instead of log1p. Best-validation checkpointing from version_6 is kept unchanged.
#
...
# aux_dim therefore grows by 4 (39 -> 43).
#
...
OUTPUT_PATH = Path("/s/chopin/e/proj/hyperspec/masfiq/models/tinyPixelViT_version_7_fusion_geohash_month_koppen_SAR_DEM_sqrt_version_6_California_North_10_2021.pth")
...
class GEDIHlsPatchDatasetFusion(Dataset):
    ...
    FIXED_BANDS = ["B02","B03","B04","B05","B06","B07","B8A","B11","B12","EVI","NDVI","NLCD","S1_VV","S1_VH"]
    ...
    def __init__(
        ...
        gridmet_dir=None,
        dem_path=None,          # CHANGE A -- new in version_7
    ):
        ...
        # CHANGE A -- new in version_7. Terrain aux features from the Copernicus DEM.
    ...
    def _load_patch_fixed_channels(self, patch_path: Path, bands_str: str | None) -> np.ndarray:
        ...
        out = np.zeros((len(self.FIXED_BANDS), raw.shape[1], raw.shape[2]), dtype=np.float32) # if 12 bands it is (12,3,3) 12 matrices each matrix has 3 rows and 3 columns
        ...
        # 0 to 10000
        # S1_VV/S1_VH are linear backscatter (typically << 2.0), so they'd be corrupted
        # by this same rule if a single bright pixel happened to push mx over 2.0.
    ...
    def __getitem__(self, idx):
        ...
        # --- CHANGE A: DEM terrain encoding (new in version_7) ---
        ...
        # 0.31, 0.82, 0.44, 0.006, 0.21, 0.75  # gridMET weather
        # 1.05, 0.34, 1.00, 0.01               # DEM: elev_km, slope, asp_sin, asp_cos
        # ]
...
def create_dataloaders(csv_path, batch_size=32, val_frac=0.2, geohash_precision=7, seed=SEED):
    ...
    ds = GEDIHlsPatchDatasetFusion(
        csv_path,
        target_col="agbd_sqrt",     # CHANGE B -- was "agbd_log" in version_6
        geohash_precision=7,
        koppen_tif_path=koppen_tif,
        koppen_legend_path=legend,
        gridmet_dir=GRIDMET_DIR,
        dem_path=DEM_PATH,          # CHANGE A -- new in version_7
    )
