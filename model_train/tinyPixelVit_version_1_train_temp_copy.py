...
def train_model(
    ...
    in_channels=in_channels, # number of bands 
    geohash_precision=geohash_precision,
    aux_dim=aux_dim,
    geohash_emb_dim=16, # Each geohash character is converted into a 16-dimensional embedding.
    embed_dim=64,# Each pixel token is projected into a 64-dimensional feature vector.Your 3×3 patch becomes 9 tokens, and each token becomes size 64.


    num_heads=4,#The transformer attention is split into 4 attention heads.
    num_layers=2, # The model uses 2 Transformer encoder layers.
    mlp_dim=128,  # Inside each Transformer layer, the feed-forward network has size 128.
    hidden_dim=256, # The final regression MLP starts with hidden size 256.
    dropout=0.2, # During training, 20% of some neurons are randomly dropped to reduce overfitting.
    ).to(device)  
