...
class GEDIHlsPatchDatasetFusion(Dataset):
    ...
    def __init__(
        ...
        self.drop_band_idxs = set()
        if drop_bands:
            for b in drop_bands:
                if b in self.FIXED_BANDS:
                    self.drop_band_idxs.add(self.FIXED_BANDS.index(b))
    ...
    def _load_patch_fixed_channels(self, patch_path: Path, bands_str: str | None) -> np.ndarray:
        ...
        # ---- Ablation: drop selected bands by zeroing them ----
        for j in self.drop_band_idxs:
            out[j] = 0.0
...
# ----------------------------
def create_dataloaders(csv_path, batch_size=32, val_frac=0.2, geohash_precision=7, seed=42, drop_bands=None):
    ...
    return train_loader, val_loader, x_img.shape[0], x_aux.shape[0]


def train_model(csv_path, num_epochs=60, batch_size=32, lr=1e-3, val_frac=0.2, geohash_precision=7, seed=42, drop_bands=None):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Device:", device, " drop_bands=", drop_bands)
    ...
    last_va_mse, last_va_mae = None, None
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        va_mse /= max(n_va, 1)
        va_mae /= max(n_va, 1)
        last_va_mse, last_va_mae = va_mse, va_mae

        print(f"Epoch {epoch:03d} | Train MSE {tr_mse:.4f} MAE {tr_mae:.4f} | Val MSE {va_mse:.4f} MAE {va_mae:.4f}")

    return model, {"val_mse": float(last_va_mse), "val_mae": float(last_va_mae)}

def run_leave_one_band_out(csv_path, out_dir, num_epochs=20, batch_size=64, lr=1e-3, seed=42):
    ...
    for b in bands:
        ...
        ckpt = out_dir / f"resnet18_drop_{b}.pth"
        torch.save(model.state_dict(), ckpt)

        results.append({"dropped_band": b, **metrics, "ckpt": str(ckpt)})
        pd.DataFrame(results).to_csv(out_dir / "ablation_leave_one_band_out.csv", index=False)

    print("Saved results:", out_dir / "ablation_leave_one_band_out.csv")

if __name__ == "__main__":
    csv_path = "/s/chopin/e/proj/hyperspec/masfiq/dataset/csv_files/gedi_field_boundary_metadata.csv"
    run_leave_one_band_out(
        csv_path=csv_path,
        out_dir="/s/chopin/e/proj/hyperspec/masfiq/models/ablations_leave_one_band_out",
        num_epochs=60,     # 12 runs x 60 epochs is huge; start with 10–20
        batch_size=64,
        lr=1e-3,
        seed=42
    )
