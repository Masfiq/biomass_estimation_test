# resnet18 version 15
# version 14 + two changes: (A) Copernicus DEM terrain aux features, (B) sqrt target
# instead of log1p. Best-validation checkpointing from version_14 is kept unchanged.
#
# ---- CHANGE A: DEM terrain features (4 new aux inputs) ----------------------------
# Adds elevation, slope and aspect (sin/cos) from the Copernicus GLO-30 DEM, sampled
# per shot by the new DEMLookup class. These go into the AUX vector alongside koppen
# and gridMET, not into the image patch, so the existing version_6 patches are reused
# as-is and no multi-day build_patches rerun is needed.
# Motivation: this study area runs from coastal redwood (~101 Mg/ha mean) to Modoc
# high desert (~14 Mg/ha) across a huge elevation gradient, and the model currently
# only reaches that gradient through the geohash embedding, i.e. by memorising WHERE
# a shot is rather than WHAT the terrain is. Measured on 2000 shots from this CSV:
#   corr(slope, AGBD) = +0.366   corr(elev, AGBD) = -0.242   corr(north-facing) = +0.100
# aux_dim therefore grows by 4.
#
# ---- CHANGE B: sqrt target instead of log1p --------------------------------------
# target_col "agbd_log" -> "agbd_sqrt" (column added by add_agbd_sqrt_column.py).
# The model trains on a transform but is scored in Mg/ha, so the back-transform's
# slope decides what a small error costs. log1p back-transforms with expm1, whose
# slope is ~y, so the penalty scales WITH biomass: a 0.1 slip costs ~37 Mg/ha at
# 368 Mg/ha and ~100 Mg/ha at 1000. sqrt back-transforms with t**2, slope 2*sqrt(y),
# so the same slip costs ~3.8 and ~6.3 Mg/ha respectively. This targets the dense
# forest errors that dominate RMSE.
#
# ---- knock-on change: Huber delta 1.0 -> 5.0 -------------------------------------
# NOT cosmetic. Huber is quadratic below delta and linear above it. In log1p space
# targets span 0.27..7.0 and typical val errors were ~0.58, comfortably inside
# delta=1.0 (quadratic regime). In sqrt space targets span 0.56..33.2, so equivalent
# errors are ~2.8 and delta=1.0 would push almost everything into the LINEAR regime,
# silently turning the loss into MAE. That would confound "sqrt vs log" with
# "MAE vs MSE-like". delta is scaled by the same ~4.75x as the target range so the
# loss keeps its shape and the comparison stays clean.

# resnet18 model
# all 14 bands "B02","B03","B04","B05","B06","B07","B8A","B11","B12","EVI","NDVI","NLCD","S1_VV","S1_VH"
...
#############changes made 
# Now we have added Huber Loss instead of MSE 
# Now we have LR scheduler , so instead of a fixed learning rate we will now move to a new learning rate when validation loss starts 
...
OUTPUT_PATH = Path("/s/chopin/e/proj/hyperspec/masfiq/models/resnet18_version_15_fusion_geohash_month_koppen_withAttentionLayer_SAR_DEM_sqrt_version_6_California_North_10_2021.pth")
...
class GEDIHlsPatchDatasetFusion(Dataset):
    ...
    def __init__(
        ...
        gridmet_dir=None,
        dem_path=None,          # CHANGE A -- new in version_15
    ):
        ...
        # CHANGE A -- new in version_15. Terrain aux features from the Copernicus DEM.
    ...
    def _load_patch_fixed_channels(self, patch_path: Path, bands_str: str | None) -> np.ndarray:
        ...
        # Scaling: scale reflectance-like bands only (NOT NLCD, NOT S1).
        # NLCD is categorical (11, 21, 95, ...) and S1_VV/S1_VH are linear backscatter
        # (typically << 2.0), so both would be corrupted by the reflectance /10000 rule
        # below if a single bright pixel happened to push mx over 2.0.
    ...
    def __getitem__(self, idx):
        ...
        # --- CHANGE A: DEM terrain encoding (new in version_15) ---
        ...
        x_aux_np = np.concatenate(
            [m_sc, koppen_onehot, gridmet_features, dem_features],
            axis=0
        ).astype(np.float32)
...
def create_dataloaders(csv_path, batch_size=32, val_frac=0.2, geohash_precision=GEOHASH_PRESITION, seed = 42):
    ...
    if "S1_VV" in ds.FIXED_BANDS and "S1_VH" in ds.FIXED_BANDS:
        ...
        print("Sample S1_VV center:", x_img[vv_idx, 2, 2].item(), flush=True)
        print("Sample S1_VH center:", x_img[vh_idx, 2, 2].item(), flush=True)
...
def train_model(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        _mem_report(f"ep{epoch:03d} train-start")
        _nb = 0
        for x_img, gh_idx, x_aux, y in train_loader:
            _nb += 1
            if _nb <= 5 or _nb % 500 == 0:
                _mem_report(f"ep{epoch:03d} batch {_nb}")
        ...
        _mem_report(f"ep{epoch:03d} val-start")
        with torch.no_grad():
