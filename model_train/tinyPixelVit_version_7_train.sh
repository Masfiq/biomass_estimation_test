#!/bin/bash
#SBATCH --job-name=biomass_attn_pvit_v7
...
#SBATCH --output=out_and_err/tinyPixelVit_version_7_train_%j.out
#SBATCH --error=out_and_err/tinyPixelVit_version_7_train_%j.err
...
# workers blows past the cgroup limit and gets OOM-killed. Version_7 adds the
# Copernicus DEM VRT as a second raster source read per sample, so this cap
# matters slightly more than before.
...
nvidia-smi
srun python -u tinyPixelVit_version_7_train.py
